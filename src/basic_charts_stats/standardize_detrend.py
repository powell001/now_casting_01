...
def readindata(features: list, reconstitute = False, removeOutlier: bool = False):
...
df1 = readindata(feat, reconstitute = False, removeOutlier=True)
...
df1.to_csv('data/standarized_detrended_gdp_other.csv')
...
def plotdata():
    ...
    axs[0].set_title("GDP, standardized, detrended, data")
    axs[1].plot(df1.loc[:, "gov_invest_std"])
    axs[1].set_title("Gov Investment, standardized, detrended, data")
    axs[2].plot(df1.loc[:, "gdp_total_detrend_diff"])
    axs[2].set_title("GDP, standardized, detrended, differenced, data")
    axs[3].plot(df1.loc[:, "gdp_total_detrend_positive"])
    axs[3].set_title("GDP, standardized, detrended, differenced, increasing/decreasing, data")

    plt.show()

plotdata()
plt.savefig("fig/standardized_detrended_gdp")
plt.close()
